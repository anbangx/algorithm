...
public class WordLadder3 {
    ...
    public ArrayList<ArrayList<String>> findLadders(String start, String end, HashSet<String> dict) {
        ...
        Queue<Node> q_start = new LinkedList<Node>();
        q_start.add(new Node(start));
        Queue<Node> q_end = new LinkedList<Node>();
        q_end.add(new Node(end));
        int preCount = 1;
        int step = 1;
        int curCount = 0;
        HashMap<String, Node> start_visited = new HashMap<String, Node>();
        HashMap<String, Node> end_visited = new HashMap<String, Node>();
        HashMap<String, Node> start_level_visited = new HashMap<String, Node>();
        HashMap<String, Node> end_level_visited = new HashMap<String, Node>();
        int minStep = Integer.MAX_VALUE;
        while (!q_start.isEmpty() || !q_end.isEmpty()) {
            String word;
            // bfs from start
            if(!q_start.isEmpty()){
                Node parent_start = q_start.poll();
                word = parent_start.val;
                for (int i = 0; i < length; i++) {
                    for (char c = 'a'; c <= 'z'; c++) {
                        StringBuilder temp = new StringBuilder(word);
                        temp.setCharAt(i, c);
                        String t = temp.toString();
                        if (!dict.contains(t) || t.equals(start.charAt(i)))
                            continue;
                        Node newNode = new Node(t);
                        newNode.parent = parent_start;
                        if(start_visited.containsKey(newNode.val) || end_visited.containsKey(newNode.val))
                            continue;
                        if (end_visited.containsKey(newNode.val)) {
                            minStep = step;
                            ArrayList<String> path = findForwardPath(newNode);
                            path.remove(path.size() - 1);
                            path.addAll(findReversePath(end_visited.get(newNode.val)));
                            results.add(path);
                            start_level_visited.put(newNode.val, newNode);
                        }
                        if (!start_visited.containsKey(newNode.val) && dict.contains(newNode.val)) { 
                            q_start.add(newNode);
                            start_level_visited.put(newNode.val, newNode);
                            curCount++;
                        }
            ...
            if(!q_end.isEmpty()){
                ...
                for (int i = 0; i < length; i++) {
                    for (char c = 'a'; c <= 'z'; c++) {
                        ...
                        if (!dict.contains(t) || t.equals(start.charAt(i)))
                            continue;
                        Node newNode = new Node(t);
                        newNode.parent = parent_end;
                        if(start_visited.containsKey(newNode.val) || end_visited.containsKey(newNode.val))
                            continue;
                        if (start_level_visited.containsKey(newNode.val) || start_visited.containsKey(newNode.val)) {
                            minStep = step;
                            ArrayList<String> path = findForwardPath(start_level_visited.get(newNode.val));
                            path.remove(path.size() - 1);
                            path.addAll(findReversePath(newNode));
                            results.add(path);
                            end_level_visited.put(newNode.val, newNode);
                        }
                        if (!end_visited.containsKey(newNode.val) && dict.contains(newNode.val)) { 
                            q_end.add(newNode);
                            end_level_visited.put(newNode.val, newNode);
                            curCount++;
                        }
                ...
                if ((--preCount) == 0) {
                    ...
                    if(step > minStep)
                        break;
                    start_visited.putAll(start_level_visited);
                    start_level_visited.clear();
                    end_visited.putAll(end_level_visited);
                    end_level_visited.clear();
                }
            }
        }
        return results;
    ...
    public ArrayList<String> findForwardPath(Node node){
    ...
    public ArrayList<String> findReversePath(Node node){
        ArrayList<String> path = new ArrayList<String>();
        while(node != null){
            path.add(node.val);
            node = node.parent;
